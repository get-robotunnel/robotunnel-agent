...
type DynReadHalf = Box<dyn AsyncRead + Send + Unpin>;
type DynWriteHalf = Box<dyn AsyncWrite + Send + Unpin>;

struct TurnTcpConn {
    reader: Mutex<DynReadHalf>,
    writer: Mutex<DynWriteHalf>,
    local_addr: SocketAddr,
    remote_addr: SocketAddr,
}

impl TurnTcpConn {
    async fn dial(url: &Url, turn_server_addr: &str) -> Result<Self> {
        let tcp = TcpStream::connect(turn_server_addr).await?;
        ...
        if url.scheme == SchemeType::Turns {
            let connector = native_tls::TlsConnector::builder()
                ...
                .build()
                .map_err(|err| Error::Other(format!("failed to build TLS connector: {err}")))?;
            let connector = TlsConnector::from(connector);
            let tls = connector
                .connect(&url.host, tcp)
                .await
                .map_err(|err| Error::Other(format!("TURN TLS handshake failed: {err}")))?;
    ...
    async fn read_framed_packet(&self, buf: &mut [u8]) -> std::result::Result<usize, util::Error> {
        ...
        if length > buf.len() {
            ...
            return Err(util::Error::Other(format!(
                "TURN TCP frame length {} exceeds buffer {}",
                length,
                buf.len()
            )));
        }
    ...
    async fn write_framed_packet(&self, buf: &[u8]) -> std::result::Result<usize, util::Error> {
        if buf.len() > u16::MAX as usize {
            return Err(util::Error::Other(format!(
                "TURN TCP frame too large: {} bytes",
                buf.len()
            )));
        }
...
impl Conn for TurnTcpConn {
    async fn connect(&self, addr: SocketAddr) -> std::result::Result<(), util::Error> {
        if addr != self.remote_addr {
            return Err(util::Error::Other(format!(
                "TURN TCP conn already connected to {}, requested {}",
    ...
    async fn recv(&self, buf: &mut [u8]) -> std::result::Result<usize, util::Error> {
        self.read_framed_packet(buf).await
    }
    ...
    pub(crate) async fn gather_candidates_relay(
        ...
        for url in urls {
            ...
            tokio::spawn(async move {
                ...
                let (loc_conn, rel_addr, rel_port, network) =
                    if url.proto == ProtoType::Udp && url.scheme == SchemeType::Turn {
                        ...
                        let network = if local_addr.is_ipv6() {
                            NetworkType::Udp6.to_string()
                        } else {
                            NetworkType::Udp4.to_string()
                        };
                        (loc_conn, rel_addr, rel_port, network)
                    } else if url.proto == ProtoType::Tcp
                        && (url.scheme == SchemeType::Turn || url.scheme == SchemeType::Turns)
                    {
                        let loc_conn = match TurnTcpConn::dial(&url, &turn_server_addr).await {
                            Ok(c) => Arc::new(c) as Arc<dyn Conn + Send + Sync>,
                            Err(err) => {
                                log::warn!(
                                    "[{}]: Failed to dial TURN server {} over {}: {}",
                                    agent_internal2.get_name(),
                                    turn_server_addr,
                                    if url.scheme == SchemeType::Turns {
                                        "tls"
                        ...
                        let rel_port = local_addr.port();
                        let network = if local_addr.is_ipv6() {
                            NetworkType::Tcp6.to_string()
                        } else {
                            NetworkType::Tcp4.to_string()
                        };
                        (loc_conn, rel_addr, rel_port, network)
                    } else {
                        log::warn!(
